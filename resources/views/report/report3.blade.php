...
<div id="search">
    <div id="searchBox">
        <form method="GET" action="{{ url('report4') }}" accept-charset="UTF-8" class="form-inline" role="search">
            ...
            <input type="text" class="form-control" style="width:370px;" placeholder="ชื่อทรัพยากร" name="search" value="{{ request('search') }}">
...
<table class="table table-bordered table-striped">
    ...
    <tr>
        ...
        <td><a href="{{ url('report3_detail?st_resource_id='.$row->id.'&'.$get) }}">{{ $row->name }}</a></td>
        <td>
            <?php 
                $st_resource_id = $row->id;

                $start_date = request('start_date');
                $end_date = request('end_date');
                
                if(!empty($start_date) and !empty($end_date)){
                    $row = $row->bookingResource->where('start_date', '>=', Date2DB($start_date))->where('start_date', '<=', Date2DB($end_date));
                }elseif(!empty($start_date) and empty($end_date)){
                    $row = $row->bookingResource->where('start_date', Date2DB($start_date));
                }elseif(empty($start_date) and !empty($end_date)){
                    $row = $row->bookingResource->where('start_date', Date2DB($end_date));
                }else{
                    $row = $row->bookingResource;
                }
            ?>
            <div>
                รออนุมัติ 
                <a href="{{ url('report3_detail?st_resource_id='.$st_resource_id.'&status=รออนุมัติ&'.$get) }}">
                    [{{ $row->where('status','รออนุมัติ')->count() }}]
                </a>
            </div>
            <div>
                อนุมัติ 
                <a href="{{ url('report3_detail?st_resource_id='.$st_resource_id.'&status=อนุมัติ&'.$get) }}">
                    [{{ $row->where('status','อนุมัติ')->count() }}]
                </a>
            </div>
            <div>
                ไม่อนุมัติ 
                <a href="{{ url('report3_detail?st_resource_id='.$st_resource_id.'&status=ไม่อนุมัติ&'.$get) }}">
                    [{{ $row->where('status','ไม่อนุมัติ')->count() }}]
                </a>
            </div>
            <div>
                ยกเลิก 
                <a href="{{ url('report3_detail?st_resource_id='.$st_resource_id.'&status=ยกเลิก&'.$get) }}">
